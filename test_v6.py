...
def merge_bounding_boxes(extracted_texts):
    ...
    for i, (bbox_i, text_i, _) in enumerate(extracted_texts):  # Unpacking adjusted here
        ...
        for j, (bbox_j, text_j, _) in enumerate(extracted_texts):  # Unpacking adjusted here
        ...
        merged_boxes.append((merged_text.strip(), merged_bbox))  # Only 2 elements here
        used_indices.add(i)

    return merged_boxes

# Function to cluster nearby bounding boxes
def cluster_nearby_bounding_boxes(extracted_texts, proximity_threshold=15):
    clustered_boxes = []
    used_indices = set()

    for i, (text_i, bbox_i) in enumerate(extracted_texts):  # Unpacking adjusted here
        if i in used_indices:
            continue
        ...
        for j, (text_j, bbox_j) in enumerate(extracted_texts):
            if i != j and j not in used_indices:
                ...
                # Check the distance in both dimensions (x and y)
                if (abs(bbox_i[0][0] - bbox_j[0][0]) < proximity_threshold or
        ...
        clustered_boxes.append((merged_text.strip(), merged_bbox))  # Only 2 elements here
        used_indices.add(i)
